...
class BaseCase(unittest.TestCase):   # 继承unittest.TestCase
    ...
    def setUpClass(cls):
        if cls.__name__ != 'BaseCase':
            cls.data_list = excel_to_list(data_file, cls.__name__)#这里读取的sheet名就是引用该类的class名字

    def get_case_data(self, case_name):
        return get_test_data(self.data_list, case_name)#return case_data

    def send_request(self, case_data):
        case_name = case_data.get('case_name')
        url = case_data.get('url')
        args = case_data.get('args')#data
